 ...
 */
fun <T: Any> Observable<T>.subscribeToViewModelObservable(action: (value: T) -> Unit): Disposable {
    return observeOn(AndroidSchedulers.mainThread()).subscribe { action(it) }
}
...
fun View.setVisibility(visible: Boolean, valueIfNotVisible: Int = View.GONE) {
